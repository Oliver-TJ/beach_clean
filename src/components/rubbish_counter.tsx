...
const RubbishCounter = () => {
    ...
    useEffect(() => {
        const query = '*[_type == "clean"]';
        setIsLoading(true);
        client.fetch<Clean[]>(query)
            .then((data) => {
                setCleans(data);
                setIsLoading(false);
            })
    ...
    // calculate total weight and find the latest clean
    const totalWeight = cleans.reduce((total, clean) => total + clean.weight, 0);
    const latestClean = cleans.length > 0
        ? [...cleans].sort((a, b) => new Date(b.cleanDate).getTime() - new Date(a.cleanDate).getTime())[0]
        : null;
    
    const finalValue = totalWeight.toFixed(1);
    ...
    return (
        <div
            {...interactionProps}
            className={`
                group relative z-50 flex w-full max-w-6xl flex-col items-center gap-4 rounded-xl 
            ...
            {latestClean && (
                ...
                >
                    ...
                    <div className="space-y-3 text-base">
                        <p className="flex items-center"><span className="mr-3 w-5 text-center">📍</span> {latestClean.location}</p>
                        <p className="flex items-center"><span className="mr-3 w-5 text-center">📅</span> {new Date(latestClean.cleanDate).toLocaleDateString('en-GB')}</p>
                        <p className="flex items-center"><span className="mr-3 w-5 text-center">👥</span> {latestClean.volunteers} volunteers</p>
                        <p className="flex items-center"><span className="mr-3 w-5 text-center">🗑️</span> {latestClean.weight}kg collected</p>
                        <p className="flex items-center"><span className="mr-3 w-5 text-center">📏</span> {latestClean.distance}km cleaned</p>
                    </div>
                </div>
            )}
        </div>
